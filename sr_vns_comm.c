...
int
sr_connect_to_server (struct sr_instance *sr, unsigned short port,
		      char *server)
{
  struct hostent *hp;
  c_open command;
...
int
sr_handle_hwinfo (struct sr_instance *sr, c_hwinfo * hwinfo)
{
  int num_entries;
  int i = 0;
...
int
sr_handle_rtable (struct sr_instance *sr, c_rtable * rtable)
{
  char fn[7 + IDSIZE + 1];
  FILE *fp;
...
int
sr_handle_auth_request (struct sr_instance *sr, c_auth_request * req)
{
...
int
sr_handle_auth_status (struct sr_instance *sr, c_auth_status * status)
{
  if (status->auth_ok)
    printf ("VNSCOMM: successfully authenticated as %s\n", sr->user);
  else
...
int
sr_read_from_server (struct sr_instance *sr /* borrowed */ )
{
  return sr_read_from_server_expect (sr, 0);
}

int
sr_read_from_server_expect (struct sr_instance *sr /* borrowed */ ,
			    int expected_cmd)
{
  int command, len;
  ...
  *((int *) buf) = htonl (len);
  ...
  while (bytes_read < len - 4)
	...
	{			/* -- just in case SIGALRM breaks recv -- */
	  errno = 0;		/* -- hacky glibc workaround -- */
	  if ((ret =
	       read (sr->sockfd, buf + 4 + bytes_read,
		     len - 4 - bytes_read)) == -1)
	    {
	      if (errno == EINTR)
		{
...
*/
    ...
    default:
      Debug ("VNSCOMM: unknown command: %d\n", command);
      break;
				...
				const char *name /* borrowed */ )
{
  ...
  iface = sr_find_interface (sr, name);
...
int
sr_send_packet (struct sr_instance *sr /* borrowed */ ,
		uint8_t * buf /* borrowed */ ,
...
void
sr_log_packet (struct sr_instance *sr, uint8_t * buf, int len)
{
...
int
sr_arp_req_not_for_us (struct sr_instance *sr, uint8_t * packet /* lent */ ,
		       unsigned int len, char *interface /* lent */ )
{
  struct sr_if *iface = sr_find_interface (sr, interface);	/* sr_get_interface(sr, interface); */
  ...
  if ((e_hdr->ether_type == htons (ETHERTYPE_ARP)) &&
      (a_hdr->ar_op == htons (ARP_REQUEST)) && (a_hdr->ar_tip != iface->ip))
    {
      return 1;
    }
